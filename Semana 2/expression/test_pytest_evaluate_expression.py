...
def test_sum_several_integers():
    expresion = "+".join([str(x) for x in range(200001)])
    assert evaluate_expression(expresion) == int(200000/2*(200000+1))


def test_substract_several_integers():
    expresion = "-".join([str(x) for x in range(200001)])
    assert evaluate_expression(expresion) == -200000/2*(200000+1)
