...
def add_king(x, y, current_board):
    ...
    new_x, new_y = move_forward(x, y, len(current_board), len(current_board[0]))
    if (within_limits):
...
def check_king(x, y, current_board):
    ...
    has_free_space = is_free(x,y,current_board) or is_free(x-1, y-1, current_board) or is_free(x, y-1, current_board) or is_free(x+1, y-1, current_board) or is_free(x-1,y, current_board) or is_free(x+1 , y ,current_board) or is_free(x-1, y+1, current_board) or is_free(x,y+1, current_board) or is_free(x+1, y+1, current_board) 
